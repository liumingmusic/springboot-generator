...
public class HBaseBeanUtil {
     ...
     */
    static <T> Put beanToPut(T obj) throws Exception {
     ...
     * @param obj 传入的实体名称
     * @param <T> obj
     * @return 获取Bean中的id, 作为Rowkey
     */
    static <T> String parseObjId(T obj) {
        Class<?> clazz = obj.getClass();
        try {
     ...
     * @throws Exception 系统异常
     */
    static <T> T resultToBean(Result result, T obj) throws Exception {
        if (result == null) {
            return null;
        }
        Class<?> clazz = obj.getClass();
